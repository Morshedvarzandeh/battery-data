...
function syncInputs() {
  ...
  $('cellHint').innerHTML =
    ...
    `<span class="prov prov-${pv.tone}" title="${esc(pv.detail)}">${pv.label}</span>` +
    (pv.contribUid ? ` <span class="mono prov-uid">${esc(pv.contribUid)}</span>` : '') +
    `<div class="prov-detail">${esc(pv.detail)}</div>`;
}
