...
def export(products: list[tuple[str, dict]], iris: dict, layers: list | None = None) -> str:
    w = Writer()
    ...
    for path, doc in products:
        ...
        index_org("org/" + maker_slug, p.get("manufacturer"))
    ...
    export_layers(w, layers or [], orgs_done, sources_done)
    return "\n".join(w.lines) + "\n"


# ---------------------------------------------------------------------
# The layers that are not products: sites as places, companies as
# organisations with their relations, supply agreements, distribution,
# market series and patents. Same provenance discipline: every claim
# hangs off a SourceLocation that quotes the document.
# ---------------------------------------------------------------------
def source_block(w: Writer, src: dict, sources_done: set) -> str:
    source_iri = iri(ID + src["uid"])
    if src["uid"] in sources_done:
        return source_iri
    pairs = [("rdf:type", "schema:CreativeWork"), ("rdf:type", "prov:Entity"),
             ("rdf:type", "bdv:Source"), ("bdv:sourceKind", lit(src.get("kind")))]
    for key, pred in (("title", "dcterms:title"), ("url", "schema:url"), ("doi", "bdv:doi"),
                      ("revision", "bdv:revision"), ("document_date", "dcterms:date"),
                      ("license", "schema:license"), ("sha256", "bdv:sha256"), ("note", "rdfs:comment")):
        if src.get(key):
            pairs.append((pred, iri(src["url"]) if key == "url" else lit(src[key])))
    if src.get("data_redistributable") is not None:
        pairs.append(("bdv:dataRedistributable", lit(bool(src["data_redistributable"]))))
    w.block(source_iri, pairs)
    sources_done.add(src["uid"])
    return source_iri


# One company, one IRI: a name is looked up against the names, legal names
# and aliases the company files and the product makers declare before a
# slug is minted, the same resolution the loader does against the database.
ORG_INDEX: dict[str, str] = {}


def index_org(uid: str, *names) -> None:
    for n in names:
        if n:
            ORG_INDEX.setdefault(str(n).strip().lower(), uid)


def org_block(w: Writer, name: str | None, uid: str | None, orgs_done: set) -> str:
    key = uid or ORG_INDEX.get((name or "").strip().lower()) or "org/" + slug(name)
    o = iri(ID + key)
    if key not in orgs_done and name:
        w.block(o, [("rdf:type", "schema:Organization"), ("schema:name", lit(name))])
        orgs_done.add(key)
    return o


def export_layers(w: Writer, docs: list[tuple[str, str, dict]], orgs_done: set, sources_done: set):
    """docs: (layer, path, document) for sites, companies, market and patents."""
    for layer, _, doc in docs:
        if layer == "companies":
            org = doc["organization"]
            index_org(org["uid"], org["name"], org.get("legal_name"), *(org.get("aliases") or []),
                      *(org.get("former_names") or []))
    for layer, path, doc in docs:
        src = doc["source"]
        source_iri = source_block(w, src, sources_done)
        loc = doc.get("locator") or {}

        if layer == "companies":
            org = doc["organization"]
            o = iri(ID + org["uid"])
            pairs = [("rdf:type", "schema:Organization"), ("rdf:type", "bdv:Company"),
                     ("schema:name", lit(org["name"]))]
            for key, pred in (("legal_name", "schema:legalName"), ("country", "schema:addressCountry"),
                              ("hq_region", "schema:addressRegion"), ("hq_locality", "schema:addressLocality"),
                              ("founded_year", "schema:foundingDate"), ("website", "schema:url"),
                              ("lei", "schema:leiCode"), ("ticker", "schema:tickerSymbol"),
                              ("ror_id", "bdv:rorId"), ("description", "schema:description")):
                if org.get(key):
                    pairs.append((pred, iri(org["website"]) if key == "website" else lit(org[key])))
            for a in org.get("aliases") or []:
                pairs.append(("schema:alternateName", lit(a)))
            for a in org.get("former_names") or []:
                pairs.append(("bdv:formerName", lit(a)))
            for r in org.get("roles") or []:
                pairs.append(("bdv:role", lit(r)))
            if org.get("parent_uid"):
                pairs.append(("schema:parentOrganization", iri(ID + org["parent_uid"])))
            pairs.append(("prov:wasDerivedFrom", location_node(loc, source_iri)))
            w.block(o, pairs)
            orgs_done.add(org["uid"])
            for i, rel in enumerate(doc.get("relations") or []):
                other = org_block(w, rel.get("organization"), rel.get("organization_uid"), orgs_done)
                pairs = [("rdf:type", "bdv:CompanyRelation"), ("bdv:from", o), ("bdv:to", other),
                         ("bdv:relation", lit(rel["relation"]))]
                for key, pred in (("share_pct", "bdv:sharePct"), ("valid_from", "schema:validFrom"),
                                  ("valid_to", "schema:validThrough"), ("notes", "rdfs:comment")):
                    if rel.get(key) is not None:
                        pairs.append((pred, lit(rel[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(rel.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"relation/{org['uid']}/{i}"), pairs)

        elif layer == "sites":
            site = doc["site"]
            s_iri = iri(ID + site["uid"])
            pairs = [("rdf:type", "schema:Place"), ("rdf:type", "bdv:Site"), ("schema:name", lit(site["name"])),
                     ("bdv:siteKind", lit(site["kind"])), ("schema:addressCountry", lit(site["country"]))]
            for key, pred in (("region", "schema:addressRegion"), ("locality", "schema:addressLocality"),
                              ("latitude", "geo:lat"), ("longitude", "geo:long"), ("status", "bdv:status"),
                              ("status_as_of", "bdv:statusAsOf"), ("deposit_type", "bdv:depositType"),
                              ("opened_year", "bdv:openedYear"), ("notes", "rdfs:comment")):
                if site.get(key) is not None:
                    pairs.append((pred, lit(site[key])))
            if site.get("operator") or site.get("operator_uid"):
                pairs.append(("bdv:operatedBy", org_block(w, site.get("operator"), site.get("operator_uid"), orgs_done)))
            for c in site.get("commodities") or []:
                pairs.append(("bdv:commodity", lit(c)))
            for p_ in site.get("products") or []:
                pairs.append(("bdv:produces", lit(p_)))
            for a in site.get("aliases") or []:
                pairs.append(("schema:alternateName", lit(a)))
            pairs.append(("prov:wasDerivedFrom", location_node(loc, source_iri)))
            w.block(s_iri, pairs)
            for i, r in enumerate(doc.get("resources") or []):
                pairs = [("rdf:type", "bdv:ResourceEstimate"), ("bdv:site", s_iri), ("bdv:commodity", lit(r["commodity"]))]
                for key, pred in (("category", "bdv:category"), ("reporting_code", "bdv:reportingCode"),
                                  ("tonnage", "bdv:tonnage"), ("tonnage_unit", "bdv:tonnageUnit"),
                                  ("grade", "bdv:grade"), ("grade_unit", "bdv:gradeUnit"),
                                  ("cutoff_grade", "bdv:cutoffGrade"), ("cutoff_unit", "bdv:cutoffUnit"),
                                  ("contained_metal", "bdv:containedMetal"), ("contained_unit", "bdv:containedUnit"),
                                  ("as_of", "bdv:asOf"), ("notes", "rdfs:comment")):
                    if r.get(key) is not None:
                        pairs.append((pred, lit(r[key])))
                for u in r.get("unstated") or []:
                    pairs.append(("bdv:unstated", lit(u)))
                pairs.append(("prov:wasDerivedFrom", location_node(r.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"resource/{site['uid']}/{i}"), pairs)
            for i, m in enumerate(doc.get("metrics") or []):
                pairs = [("rdf:type", "bdv:SiteMetric"), ("bdv:site", s_iri), ("bdv:metric", lit(m["metric"])),
                         ("bdv:value", lit(m["value"])), ("bdv:unit", lit(m["unit"])),
                         ("bdv:periodStart", lit(m["period_start"]))]
                for key, pred in (("subject", "bdv:subject"), ("status", "bdv:status"), ("period_end", "bdv:periodEnd"),
                                  ("as_of", "bdv:asOf"), ("notes", "rdfs:comment")):
                    if m.get(key) is not None:
                        pairs.append((pred, lit(m[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(m.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"metric/{site['uid']}/{i}"), pairs)
            for i, o_ in enumerate(doc.get("ownership") or []):
                owner = org_block(w, o_.get("organization"), o_.get("organization_uid"), orgs_done)
                pairs = [("rdf:type", "bdv:Ownership"), ("bdv:site", s_iri), ("bdv:owner", owner)]
                for key, pred in (("share_pct", "bdv:sharePct"), ("role", "bdv:role"),
                                  ("valid_from", "schema:validFrom"), ("valid_to", "schema:validThrough")):
                    if o_.get(key) is not None:
                        pairs.append((pred, lit(o_[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(o_.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"ownership/{site['uid']}/{i}"), pairs)

        elif layer == "market":
            stem = slug(src["uid"].split("/", 1)[1])
            for i, r in enumerate(doc.get("commodity_prices") or []):
                pairs = [("rdf:type", "schema:PriceSpecification"), ("rdf:type", "bdv:CommodityPrice"),
                         ("bdv:commodity", lit(r["commodity"])), ("schema:price", lit(r["value"])),
                         ("schema:priceCurrency", lit(r["currency"])), ("schema:unitText", lit(r["per_unit"])),
                         ("schema:validFrom", lit(r["period_start"])), ("schema:validThrough", lit(r["period_end"])),
                         ("bdv:market", lit(r["market"]))]
                for key, pred in (("traded_form", "bdv:tradedForm"), ("grade", "bdv:grade"), ("basis", "bdv:basis"),
                                  ("provider", "bdv:provider")):
                    if r.get(key):
                        pairs.append((pred, lit(r[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(r.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"price/{stem}/{i}"), pairs)
            for i, r in enumerate(doc.get("price_indices") or []):
                pairs = [("rdf:type", "schema:PriceSpecification"), ("rdf:type", "bdv:PriceIndex"),
                         ("bdv:segment", lit(r["segment"])), ("schema:price", lit(r["value"])),
                         ("schema:priceCurrency", lit(r["currency"])), ("schema:unitText", lit(r.get("per_unit") or "kWh")),
                         ("bdv:region", lit(r["region"])), ("schema:validFrom", lit(r["period_start"])),
                         ("schema:validThrough", lit(r["period_end"]))]
                for key, pred in (("chemistry_family", "bdv:chemistryFamily"), ("chemistry", "bdv:chemistry"),
                                  ("sector", "bdv:sector"), ("basis", "bdv:basis"), ("provider", "bdv:provider")):
                    if r.get(key):
                        pairs.append((pred, lit(r[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(r.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"index/{stem}/{i}"), pairs)
            for i, r in enumerate(doc.get("market_volumes") or []):
                pairs = [("rdf:type", "bdv:MarketVolume"), ("bdv:metric", lit(r["metric"])), ("bdv:value", lit(r["value"])),
                         ("bdv:unit", lit(r["unit"])), ("bdv:periodStart", lit(r["period_start"])),
                         ("bdv:periodEnd", lit(r["period_end"]))]
                if r.get("organization") or r.get("organization_uid"):
                    pairs.append(("bdv:organization", org_block(w, r.get("organization"), r.get("organization_uid"), orgs_done)))
                for key, pred in (("region", "bdv:region"), ("country", "schema:addressCountry"), ("sector", "bdv:sector"),
                                  ("chemistry_family", "bdv:chemistryFamily"), ("chemistry", "bdv:chemistry"),
                                  ("share_pct", "bdv:sharePct"), ("rank", "bdv:rank"), ("provider", "bdv:provider")):
                    if r.get(key) is not None:
                        pairs.append((pred, lit(r[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(r.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"volume/{stem}/{i}"), pairs)
            for i, r in enumerate(doc.get("trade_flows") or []):
                pairs = [("rdf:type", "bdv:TradeFlow"), ("bdv:reporter", lit(r["reporter_country"])),
                         ("bdv:partner", lit(r["partner_country"])), ("bdv:hsCode", lit(r["hs_code"])),
                         ("bdv:direction", lit(r["direction"])), ("bdv:periodStart", lit(r["period_start"])),
                         ("bdv:periodEnd", lit(r["period_end"]))]
                for key, pred in (("commodity", "bdv:commodity"), ("value_usd", "bdv:valueUsd"),
                                  ("quantity", "bdv:quantity"), ("quantity_unit", "bdv:quantityUnit")):
                    if r.get(key) is not None:
                        pairs.append((pred, lit(r[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(r.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"trade/{stem}/{i}"), pairs)
            for a in doc.get("supply_agreements") or []:
                sup = org_block(w, a.get("supplier"), a.get("supplier_uid"), orgs_done)
                buy = org_block(w, a.get("buyer"), a.get("buyer_uid"), orgs_done)
                pairs = [("rdf:type", "bdv:SupplyAgreement"), ("schema:seller", sup), ("schema:buyer", buy),
                         ("bdv:subject", lit(a["subject"]))]
                if a.get("site_uid"):
                    pairs.append(("bdv:site", iri(ID + a["site_uid"])))
                for key, pred in (("kind", "bdv:agreementKind"), ("traded_form", "bdv:tradedForm"), ("volume", "bdv:volume"),
                                  ("volume_unit", "bdv:volumeUnit"), ("volume_period", "bdv:volumePeriod"),
                                  ("valid_from", "schema:validFrom"), ("valid_to", "schema:validThrough"),
                                  ("announced_on", "bdv:announcedOn"), ("notes", "rdfs:comment")):
                    if a.get(key) is not None:
                        pairs.append((pred, lit(a[key])))
                pairs.append(("prov:wasDerivedFrom", location_node(a.get("locator") or {}, source_iri)))
                w.block(iri(ID + a["uid"]), pairs)
            for i, d in enumerate(doc.get("distributions") or []):
                dist = org_block(w, d.get("distributor"), d.get("distributor_uid"), orgs_done)
                pairs = [("rdf:type", "bdv:Distribution"), ("bdv:distributor", dist)]
                if d.get("manufacturer") or d.get("manufacturer_uid"):
                    pairs.append(("bdv:manufacturer", org_block(w, d.get("manufacturer"), d.get("manufacturer_uid"), orgs_done)))
                for key, pred in (("status", "bdv:status"), ("url", "schema:url"), ("valid_from", "schema:validFrom"),
                                  ("valid_to", "schema:validThrough")):
                    if d.get(key):
                        pairs.append((pred, iri(d["url"]) if key == "url" else lit(d[key])))
                for r_ in d.get("regions") or []:
                    pairs.append(("bdv:region", lit(r_)))
                for f_ in d.get("product_families") or []:
                    pairs.append(("bdv:productFamily", lit(f_)))
                pairs.append(("prov:wasDerivedFrom", location_node(d.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"distribution/{stem}/{i}"), pairs)

        elif layer == "patents":
            fam = doc["family"]
            f_iri = iri(ID + fam["uid"])
            pairs = [("rdf:type", "bdv:PatentFamily"), ("dcterms:title", lit(fam["title"])),
                     ("bdv:docdbFamilyId", lit(fam["docdb_family_id"])),
                     ("bdv:primaryCategory", lit(fam["primary_category"]))]
            for key, pred in (("abstract", "dcterms:abstract"), ("earliest_priority_date", "bdv:earliestPriorityDate")):
                if fam.get(key):
                    pairs.append((pred, lit(fam[key])))
            pairs.append(("prov:wasDerivedFrom", location_node(loc, source_iri)))
            w.block(f_iri, pairs)
            for p_ in doc["publications"]:
                pu = iri(ID + f"{fam['uid']}/{p_['publication_number']}")
                pairs = [("rdf:type", "schema:CreativeWork"), ("rdf:type", "bdv:PatentPublication"),
                         ("bdv:family", f_iri), ("bdv:publicationNumber", lit(p_["publication_number"])),
                         ("bdv:jurisdiction", lit(p_["jurisdiction"])), ("dcterms:title", lit(p_["title"])),
                         ("schema:url", iri(p_["publication_url"]))]
                for key, pred in (("kind_code", "bdv:kindCode"), ("application_number", "bdv:applicationNumber"),
                                  ("abstract", "dcterms:abstract"), ("priority_date", "bdv:priorityDate"),
                                  ("filing_date", "bdv:filingDate"), ("publication_date", "schema:datePublished"),
                                  ("grant_date", "bdv:grantDate"), ("legal_status", "bdv:legalStatus"),
                                  ("legal_status_jurisdiction", "bdv:legalStatusJurisdiction"),
                                  ("legal_status_as_of", "bdv:legalStatusAsOf")):
                    if p_.get(key):
                        pairs.append((pred, lit(p_[key])))
                for a in p_.get("applicants") or []:
                    pairs.append(("bdv:applicant", lit(a)))
                for a in p_.get("assignees") or []:
                    pairs.append(("bdv:assignee", lit(a)))
                for a in p_.get("inventors") or []:
                    pairs.append(("bdv:inventor", lit(a)))
                for c in p_.get("categories") or []:
                    pairs.append(("bdv:primaryCategory" if c.get("primary") else "bdv:category", lit(c["code"])))
                pairs.append(("prov:wasDerivedFrom", location_node(p_.get("locator") or {}, source_iri)))
                w.block(pu, pairs)
            for i, l in enumerate(doc.get("links") or []):
                pairs = [("rdf:type", "bdv:PatentLink"), ("bdv:family", f_iri), ("bdv:relation", lit(l["relation"]))]
                if l.get("product_uid"):
                    pairs.append(("bdv:target", iri(ID + l["product_uid"])))
                elif l.get("material_uid"):
                    pairs.append(("bdv:target", iri(ID + l["material_uid"])))
                else:
                    pairs.append(("bdv:target", org_block(w, l.get("organization"), l.get("organization_uid"), orgs_done)))
                if l.get("confidence") is not None:
                    pairs.append(("bdv:confidence", lit(l["confidence"])))
                pairs.append(("prov:wasDerivedFrom", location_node(l.get("locator") or {}, source_iri)))
                w.block(iri(ID + f"patentlink/{fam['uid']}/{i}"), pairs)


LAYER_DIRS = {"sites": "sites", "companies": "companies", "market": "market", "patents": "patents"}


def layer_documents() -> list[tuple[str, str, dict]]:
    """(layer, path, doc) for every non-product contribution, in a stable order."""
    sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
    from validate_layers import layer_of  # noqa: E402
    docs = []
    for layer in ("companies", "sites", "market", "patents"):
        for f in sorted(glob.glob(os.path.join(ROOT, "contrib", LAYER_DIRS[layer], "**", "*.y*ml"), recursive=True)):
            with open(f, encoding="utf-8") as fh:
                doc = yaml.safe_load(fh)
            if layer_of(doc) == layer:
                docs.append((layer, f, doc))
    return docs
...
def main() -> int:
    ...
    body = export(products, iris, layers)
    if a.check:
    ...
    print(f"  wrote {os.path.relpath(OUT_TTL, ROOT)}: {len(products)} products, {n_obs} observations, "
          f"{len(layers)} site/company/market/patent file(s), {len(body):,} bytes")
    if a.jsonld:
