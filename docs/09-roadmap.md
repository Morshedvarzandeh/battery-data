...
## 10. The whole landscape, named

The owner asked for the whole data set: every cell maker, every cathode and
anode maker, every gigafactory. Naming them is a recall; proving them needs a
document for each. Both belong here, and they must never be confused.

- [x] **10.1 A candidate format that cannot look sourced.** A name, a country, a role or a site kind, the page to verify it against, and how sure the recall is. No source, no page, no quote, no capacity, no tonnage, refused by the JSON schema, the validator and a database CHECK alike.
- [x] **10.2 The sets.** Cell and battery makers; cathode, anode, precursor, electrolyte, separator and foil makers with their plants; cell, module and component factories; mines, refineries, recyclers, second-life companies and test laboratories.
- [x] **10.3 Staging and the queue as a query.** bd_stage.layer_candidate with the CHECK, a view carrying the supply-chain stage each name sits on, and an API resource in a layer of its own that says it is not accepted data.
- [x] **10.4 Verification.** A tool that fetches the page a candidate names, finds the name in it and writes a contribution whose locator quotes the sentence, saying in the record which fields rest on the quote and which are still recalled.
- [x] **10.5 Coverage.** A third state on the coverage list, named-and-queued, between sourced and not covered, so knowing what to look for is never mistaken for having it.
- [x] **10.6 Tests and documentation.** The whole path exercised offline against saved pages, the honesty rules tested, and docs/14-candidates.md.
...
- 2026-09-04 · **10.1** · json-schema/layer-candidate.schema.json with additionalProperties false, tools/validate_layer_candidates.py refusing any provenance-shaped key however nested, and a CHECK on bd_stage.layer_candidate refusing a payload that carries a quote, page, locator or source. Every set declares who recalled it, on what date, and a caveat naming what it is; every entry carries its own confidence and the https page to verify it against.
- 2026-09-04 · **10.2** · Four sets, 255 companies and 252 sites: cell-makers (CATL to the lead-acid and supercapacitor makers, 128 companies across China, Korea, Japan, Taiwan, Europe, the United States and India), active-material-makers (61 cathode, anode, precursor, electrolyte, separator and foil makers with 56 of their plants), gigafactories (142 cell, module and component factories with the joint ventures and car makers that operate them), upstream-and-recycling (57 miners, refiners, recyclers, second-life companies, test laboratories and certification bodies with 54 of their sites). Statuses carry a warning: Northvolt's bankruptcy, ACC's paused plants, FREYR becoming T1 Energy, Natron winding down and Cellforce being scaled back are all noted rather than asserted.
- 2026-09-04 · **10.3** · bd_stage.layer_candidate and bd_stage.v_layer_candidate, which carries the supply-chain stage each role or site kind sits on and whether the uid is already in the library; tools/load_layer_candidates.py stages a set idempotently under an ingest job keyed by the file's sha256. The API registry gains a schema and an accepted flag, and /v1/layer_candidates answers in a layer of its own whose every response carries accepted false and a warning.
- 2026-09-04 · **10.4** · tools/verify_layer_candidates.py fetches the page a candidate names, turns block tags and menu separators into sentence boundaries, scores every sentence naming the company and writes a contribution whose source carries the URL, the retrieval date and the sha256 and whose locator quotes the winning sentence. Fields the page did not confirm say so in the record. A name not on the page produces nothing and stays in the queue.
- 2026-09-04 · **10.5** · web/data/coverage.json targets are matched to the queue by uid or by a normalised name, and the Coverage tab shows a third state, named and awaiting a document, between sourced and not covered: 56 sourced, 76 queued, 368 not covered of 500 targets.
- 2026-09-04 · **10.6** · tests/test_layer_candidates.py runs the whole path offline against saved pages under tests/fixtures: the sets validate, none carries provenance or a capacity, the verifier turns a name on a page into a contribution that passes tools/validate_layers.py, a name not on the page is refused, the quote is prose rather than a navigation bar, and two pages on one host are two sources. docs/14-candidates.md explains the gate; CI stages the queue and proves the database still refuses a candidate carrying a quote.
